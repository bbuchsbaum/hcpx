...
> **Note**: The HCP S3 bucket is requester-pays. You’ll typically need AWS
> credentials configured to download files, and downloads may incur AWS charges.
...
6. **If using BALSA/Aspera**: Download+extract to a local mirror, then use `backend = "local"` and `catalog_build(..., scan_s3 = TRUE)` to index the mirror

### Data Analysis
7. **Use `load_run()` for complete runs**: Gets dtseries, confounds, and EVs together
8. **Concatenate with care**: Use `run_boundaries` for per-run normalization
9. **Cache derived products**: Use `derive()` to avoid recomputing parcellations
10. **Prefer parcellation**: Reduces data size and noise for connectivity analyses
11. **Check coordinates**: MNI coordinates are automatically converted to voxel space
